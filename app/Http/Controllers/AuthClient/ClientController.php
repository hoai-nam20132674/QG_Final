...
class ClientController extends Controller
{
    ...
    public function index(){
        ...
        $products_highlight = Products::join('images_products', 'products.id', '=', 'images_products.products_id')->join('products_detail', 'products.id', '=', 'products_detail.products_id')->where('products.display',1)->where('products.highlights',1)->where('images_products.role',1)->orderBy('products.updated_at', 'DESC')
            ...
            ->take(8)->get();
        $products_sale = Products::join('images_products', 'products.id', '=', 'images_products.products_id')->join('products_detail', 'products.id', '=', 'products_detail.products_id')->where('products.display',1)->where('images_products.role',1)->orderBy('products.updated_at', 'DESC')
            ->select('products.*', 'images_products.url AS avatar','products_detail.price AS maxPrice','products_detail.products_id','products_detail.id AS products_detail_id','products_detail.sale')
            ->take(8)->get();
        $products_new = Products::join('images_products', 'products.id', '=', 'images_products.products_id')->join('products_detail', 'products.id', '=', 'products_detail.products_id')->where('products.display',1)->where('images_products.role',1)->orderBy('products.updated_at', 'DESC')
            ->select('products.*', 'images_products.url AS avatar','products_detail.price AS maxPrice','products_detail.products_id','products_detail.id AS products_detail_id','products_detail.sale')
            ->take(8)->get();
        $productsGroup = $this->groupProduct($products_highlight);
        $products_highlight = $this->filterProduct($productsGroup);
        $productsGroup = $this->groupProduct($products_sale);
        $products_sale = $this->filterProduct($productsGroup);
        $productsGroup = $this->groupProduct($products_new);
        $products_new = $this->filterProduct($productsGroup);

    	return view('front-end-qg.page-content.home',['system'=>$system,'cates'=>$cates,'products_new'=>$products_new,'products_sale'=>$products_sale,'products_highlight'=>$products_highlight,'cates_highlight'=>$cates_highlight,'blogs'=>$blogs,'slides'=>$slides,'menus'=>$menus]);
    ...
    public function rootPageContent($url) {
    	...
    	if(!$cates->isEmpty()){
            ...
            $products_highlight = $this->filterProduct($productsGroup);
            $cate = Categories::where('url',$url)->get()->first();
    		return view('front-end-qg.page-content.categorie',['system'=>$system,'cates'=>$cates,'cate'=>$cate,'products'=>$products,'menus'=>$menus,'products_highlight'=>$products_highlight]);
        ...
        if(!$products->isEmpty()){
            ...
            return view('front-end-qg.page-content.product',['system'=>$system,'cates'=>$cates,'products'=>$products,'images'=>$images,'properties_type'=>$properties_type,'properties'=>$properties, 'cate_parent'=>$cate_parent,'products_same'=>$products_same,'menus'=>$menus]);
            // dd($content);
        ...
        if(!$blogs->isEmpty()){
            ...
            $menus = Menus::select()->orderBy('stt','ASC')->get();
            $blog = Blogs::where('url',$url)->get()->first();
            return view('front-end.page-content.error',['system'=>$system,'cates'=>$cates,'menus'=>$menus]);
        }
