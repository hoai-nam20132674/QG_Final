...
class Products extends Model
{
    ...
    public function addProduct($request){
        ...
        if(Input::hasFile('image_detail')){
            foreach(Input::file('image_detail') as $file){
                if(isset($file)){
                    ...
                    $file->move('public/uploads/images/products/detail/',$file_name);
    	...
    	$request->file('share_image')->move('public/uploads/images/products/image_share/',$image_share);
        $img_share = new ImageShare;
        $img_share->url = $image_share;
        $img_share->save();
        $request->file('avatar')->move('public/uploads/images/products/avatar/',$avatar);
    ...
    public function editProduct($request,$id){
        ...
        if(Input::hasFile('share_image')){
            ...
            $request->file('share_image')->move('public/uploads/images/products/image_share/',$file_name); 
        }
        if(Input::hasFile('avatar')){
            $ava_name = $request->file('avatar')->getClientOriginalName();
            $request->file('avatar')->move('public/uploads/images/products/avatar/',$ava_name);
    ...
    public function addProductDetailLog($productDetail){
        ...
        $prDL->content = '<p>Khởi tạo sản phẩm chi tiết</p><p>Người thực hiện: '.$userName.'</p><p>Giá: '.$productDetail->price.'</p><br/><p>Giá sale: '.$productDetail->sale.'</p><br/><p>Số lượng: '.$productDetail->amount.'</p>';
        $prDL->save();
        return true;
    }
